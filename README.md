...
Lightweight dependency-injection container for TypeScript/JavaScript

Based on [tsyringe-neo](https://github.com/risen228/tsyringe-neo) (which is forked from [tsyringe](https://github.com/microsoft/tsyringe)).

## diff `djin` → `tsyringe-neo`

- Additional helper functions on `DependencyContainer`:
  - `registeredTokens()`
  - `afterRegistration(...)`
...
- JSR support
- Features being added as needed/requested

## diff `tsyringe-neo` → `tsyringe`
...
```sh
$ deno add jsr:@superepic/djin
$ bunx jsr add @superepic/djin
$ npx jsr add @superepic/djin
$ yarn dlx jsr add @superepic/djin
$ pnpm dlx jsr add @superepic/djin
```
...
Djin performs [Constructor Injection](https://en.wikipedia.org/wiki/Dependency_injection#Constructor_injection)
...
runtime. Djin relies on several decorators in order to collect metadata about classes
to be instantiated.

#### Usage

```typescript
import {injectable} from "djin";
...
```typescript
import {autoInjectable} from "djin";
...
```typescript
import {injectable, inject} from "djin";
...
```typescript
import {injectable, injectAll} from "djin";
...
```typescript
import {instanceCachingFactory} from "djin";
...
```typescript
import {instancePerContainerCachingFactory} from "djin";
...
```typescript
import {predicateAwareClassFactory} from "djin";
...
`afterRegistration` is used to take an action after an object is registered.
...
container.afterRegistration(
  Bar,
  () => {
    console.log("Bar has been registered!")
  }
)
```
...
We can rest in the fact that a `DelayedConstructor` could be used in the same contexts that a constructor and will be handled transparently by Djin. Such idea is used in the next example involving interfaces:
...
```typescript
import {singleton, inject} from "djin";
